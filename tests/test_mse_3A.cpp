...
//==================================================================================================
//  Unit tests for fixed-size, statically allocated matrix engine, Case 3A (column matrix)
//==================================================================================================
//
using mse_f_ffv_cm = matrix_storage_engine<float, extents<size_t, 4, 1>, void, matrix_layout::column_major>;
using mse_f_ffv_rm = matrix_storage_engine<float, extents<size_t, 4, 1>, void, matrix_layout::row_major>;
using mse_i_ffv_cm = matrix_storage_engine<int,   extents<size_t, 4, 1>, void, matrix_layout::column_major>;
using mse_d_ffv_rm = matrix_storage_engine<double, extents<size_t, 4, 1>, void, matrix_layout::row_major>;

using msupport = matrix_engine_support;

TEST(MSE_Matrix_3A, DefaultCtor)
{
    mse_f_ffv_rm    e1;
...
TEST(MSE_Matrix_3A, CmpEq)
{
    ...
    //- Verify equality against an init-list, array, and mdspan of equivalent values.
    //
    ...
    //- Verify inequality against an init-list, array, and mdspan of different values.
    //
    ...
    EXPECT_FALSE(msupport::compare(e3, mse_f_ffv_rm(LST_41_1)));
    EXPECT_FALSE(msupport::compare(e3, a4));
    EXPECT_FALSE(msupport::compare(e3, s4));
}


TEST(MSE_Matrix_3A, MoveCtor)
{
...
TEST(MSE_Matrix_3A, ListCtor)
{
    //- Construct new engines via list ctor and verify that their initial states.
    //
    mse_f_ffv_rm    e1 = LST_41_1;
    mse_f_ffv_rm    e2 LST_41_1 ;
    mse_f_ffv_rm    e3(LST_41_1);
...
TEST(MSE_Matrix_3A, EngineAssign)
{
    //- Default construct and verify initial state.
    //
    mse_f_ffv_rm    e1, e2;
...
TEST(MSE_Matrix_3A, OtherAssign)
{
    //- Default construct and verify initial state.
    //
    mse_f_ffv_rm     e1;
...
TEST(MSE_Matrix_3A, Span)
{
    mse_f_ffv_rm        e1(il_41_2), e2, e3{e1};
