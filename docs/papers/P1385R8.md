...
several participants in SG14 Linear Algebra SIG conference calls. And for a
given arithmetical expression, they will expect run-time computational
...
further discussions and refinements, and to serve as a guide for those
attempting to build implementations from this specification.
...
1. **Operators**, which provide the desired mathematical syntax and carry out
the intended arithmetic upon `matrix` objects.

At a lower level are a number of supporting traits types employed by the
proposal to determine the return type of the operator and perform the
corresponding arithmetic operation.  There are several such traits types:
...
arithmetic operation involving `matrix` operands.  As part of that process,
this traits type uses the element promotion traits to determine the element
type of the resulting engine.

+ **Arithmetic traits** determine the type and value of a `matrix` resulting
from an arithmetical operation.  As part of that process, this traits type
uses the engine promotion traits to determine the engine type of the resulting
`matrix`.  Having determined the result type, the arithmetic traits also have
a member function that carries out the actual computations.
...
to use for computing its result.  The proposed traits class template
`std::math::matrix_operation_traits_selector` is a library customization
point.
...
+ Parameter name `MVT` represents a view engine's functionality type
(e.g., transpose, row, column, submatrix, etc.).
...
//- Engine types.
//
template<class T>                                           struct matrix_scalar_engine;
template<class T, size_t R, size_t C, class AT, class LT>   class matrix_storage_engine;
...
using dynamic_column_vector =
    matrix<matrix_storage_engine<T, dynamic_extent, 1, allocator<T>, matrix_layout::column_major>, COT>;

template<class T, class COT = void>
using dynamic_row_vector =
    matrix<matrix_storage_engine<T, 1, dynamic_extent, allocator<T>, matrix_layout::row_major>, COT>;
