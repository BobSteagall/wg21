...
During the Prague meeting, a vote in support of arithmetic customisation was
taken and no consensus was achieved. The status quo remains including support
for arithmetic customisation, but stronger justification is clearly needed.
...
## Vector multiplication and operation customisation

Also during the Prague meeting, a vote in support of vector-vector
multiplication via operator* was taken and no consensus was achieved. Again,
the status quo remains including vector-vector multiplication, but again,
stronger justification is needed.

The root of the reviewer concern is the ambiguity of what multiplication of
two vectors can mean: is it an outer product or an inner product? The current
design of the interface now identifies row and column vectors, which means
that the meaning of the multiplication now depends on the types being
multiplied, which is the normal mechanism of operator overloading. There is
no ambiguity as long as the user is aware of the types they are multiplying.
...
+ Parameter names `T`, `T1`, `T2` represent element types.

+ Parameter names `U`, `V`, and `W`

+ Parameter names `ET`, `ET1`, and `ET2` represent engine types.

+ Parameter names `L`, `LT1`, and `LT2` represent layout types.

+ Parameter names `COT`, `COT1`, and `COT2` represent customized operation
traits types.

+ Parameter names `OT`, and `OTR` represent operation traits types.

+ Parameter names `OP1` and `OP2` represent the operand types deduced by an
arithmetic operator.

+ Parameter names `A`, `AT1`, and `AT2` represent allocator types.

+ Parameter name `N` represents the number of elements in a fixed-size vector
or vector engine.

+ Parameter name `C` represents the number of columns in a fixed-size matrix
or matrix engine.

+ Parameter name `R` represents the number of rows in a fixed-size matrix or
matrix engine.

+ Parameter name `X` represents the extents
...
to emphasize the difference betweeen the mathematical vector type described in this proposal, 
`std::math::basic_vector`, and the general-purpose container type `std::vector`.
...
template <class ET, class COT> class matrix;
...
template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr bool operator ==(matrix<ET1, COT1> const& m1, matrix<ET2, COT2> const& m2);
...
template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr auto operator +(matrix<ET1, COT1> const& m1, matrix<ET2, COT2> const& m2);
...
template<class ET1, class COT1, class S2> 
    inline constexpr auto operator *(vector<ET1, COT1> const& v1, S2 const& s2);
template<class S1, class ET2, class COT2> 
    inline constexpr auto operator *(S1 const& s1, vector<ET2, COT2> const& v2);
template<class ET1, class COT1, class S2> 
    inline constexpr auto operator *(matrix<ET1, COT1> const& m1, S2 const& s2);
template<class S1, class ET2, class COT2> 
    inline constexpr auto operator *(S1 const& s1, matrix<ET2, COT2> const& m2);

template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr auto operator *(basic_vector<ET1, COT1> const& v1, basic_vector<ET2, COT2> const& v2);
template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr auto operator *(basic_vector<ET1, COT1> const& m1, matrix<ET2, COT2> const& m2);
template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr auto operator *(matrix<ET1, COT1> const& v1, basic_vector<ET2, COT2> const& v2);
template<class ET1, class COT1, class ET2, class COT2>
    inline constexpr auto operator *(matrix<ET1, COT1> const& m1, matrix<ET2, COT2> const& m2);
...
template<class OT, class U, class V>          using get_addition_element_traits_t;  
template<class OT, class U, class V, class W> using get_addition_allocation_traits_t<OT, U, V, W>;
template<class OT, class U, class V>          using get_addition_layout_traits_t;
template<class OT, class U, class V>          using get_addition_engine_traits_t;
template<class OT, class U, class V>          using get_addition_arithmetic_traits_t;

template<class OT, class U, class V>          using get_subtraction_element_traits_t;
template<class OT, class U, class V, class W> using get_subtraction_allocation_traits_t<OT, U, V, W>
template<class OT, class U, class V>          using get_subtraction_layout_traits_t;
template<class OT, class U, class V>          using get_subtraction_engine_traits_t;
template<class OT, class U, class V>          using get_subtraction_arithmetic_traits_t;

template<class OT, class U, class V>          using get_multiplication_element_traits_t;
template<class OT, class U, class V, class W> using get_multiplication_allocation_traits_t<OT, U, V, W>
template<class OT, class U, class V>          using get_multiplication_layout_traits_t;
template<class OT, class U, class V>          using get_multiplication_engine_traits_t;
template<class OT, class U, class V>          using get_multiplication_arithmetic_traits_t;

template<class OT, class U, class V>          using get_division_element_traits_t;
template<class OT, class U, class V, class W> using get_division_allocation_traits_t<OT, U, V, W>
template<class OT, class U, class V>          using get_division_layout_traits_t;
template<class OT, class U, class V>          using get_division_engine_traits_t;
template<class OT, class U, class V>          using get_division_arithmetic_traits_t;
...
The over-arching purpose of the engine types is to perform resource management on behalf of an associated MathObj
instance that owns the engine. At a minimum, all of the engine types provide a basic interface for const element
indexing, determining row and column sizes, and determining row and column capacities. They also export public
type aliases which specify their element type, whether or not they are dense, whether or not they are rectangular,
whether or not they are resizable, whether or not their memory layout is row-major, and a 2-tuple for describing sizes
and capacities.

It is important to note that an engine’s resource management duties are primarily related to storage. To that end, an
engine may own the storage it manages and control its lifetime, or it may be non-owning and represent a view of
storage owned by some other object.

One can also imagine engines that manage resources related to execution. This is an area of ongoing work and not
yet addressed in this proposal.
...
The library defines the following set of partial specializations.
    ...
    class matrix_storage_engine<T, extents<N>, A, L>
{
    ...
    using storage_type    //- Implementation-defined
    using mdspan_traits   //- Implementation-defined
    using support_traits  //- Implementation-defined
...
template<class ET> class matrix_view_engine<ET, matrix_view::const_negation>
{
    ...
    using support_traits        //- Implementation-defined
    using const_mdspan_traits   //- Implementation-defined

    static constexpr bool   has_mdspan = const_mdspan_traits::has_mdspan;

  public:
    using engine_type        = ET;
    using owning_engine_type   //- Implementation-defined
...
{
    static constexpr bool   is_cx =   //- Implementation-defined
...
class matrix
{
  public:
    ...
    using transpose_type       = matrix<matrix_view_engine<engine_type, /* See note */>, COT>;
    using const_transpose_type = matrix<matrix_view_engine<engine_type, matrix_view::const_transpose>, COT>;

    using column_type          = matrix<matrix_view_engine<engine_type, /* See note */>, COT>;
    using const_column_type    = matrix<matrix_view_engine<engine_type, matrix_view::const_column>, COT>;

    using row_type             = matrix<matrix_view_engine<engine_type, /* See note */>, COT>;
    using const_row_type       = matrix<matrix_view_engine<engine_type, matrix_view::const_row>, COT>;

    using submatrix_type       = matrix<matrix_view_engine<engine_type, /* See note */>, COT>;
  ...
  public:
    ...
    constexpr explicit matrix(matrix<ET2, COT2> const& rhs);

    //----------------------------------------------------------
    //- Construction from a 2D mdspan.
    //
    template<class U, ptrdiff_t X0, ptrdiff_t X1, class L, class A>
    constexpr explicit matrix(basic_mdspan<U, extents<X0, X1>, L, A> const& rhs);
    ...
    //
    template<class ET2, class COT2>
    constexpr explicit matrix(basic_vector<ET2, COT2> const& rhs);

    //----------------------------------------------------------
    //- Construction from a standard random-access container.
    //
    template<class CT>
    constexpr explicit matrix(CT const& rhs);

    //----------------------------------------------------------
    //- Construction from a 1D mdspan.
    //
    template<class U, ptrdiff_t X0, class L, class A>
    constexpr explicit matrix(basic_mdspan<U, extents<X0>, L, A> const& rhs);

    //----------------------------------------------------------
    //- Construction from a 1D initialization list.
    //
    template<class U>
    constexpr matrix(initializer_list<U> rhs);

    //----------------------------------------------------------
    //- Assignment from a different matrix engine type.
    //
    template<class ET2, class COT2>
    constexpr matrix& operator =(matrix<ET2, COT2> const& rhs);

    //----------------------------------------------------------
    //- Assignment from a 2D mdspan.
    //
    template<class U, ptrdiff_t X0, ptrdiff_t X1, class L, class A>
    constexpr matrix& operator =(basic_mdspan<U, extents<X0, X1>, L, A> const& rhs);
    ...
    //
    template<class ET2, class COT2>
    constexpr matrix& operator =(basic_vector<ET2, COT2> const& rhs);

    //----------------------------------------------------------
    //- Assignment from a standard random-access container.
    //
    template<class CT>
    constexpr matrix& operator =(CT const& rhs);

    //----------------------------------------------------------
    //- Assignment from a 1D mdspan.
    //
    template<class U, ptrdiff_t X0, class L, class A>
    constexpr matrix& operator =(basic_mdspan<U, extents<X0>, L, A> const& rhs);

    //----------------------------------------------------------
    //- Assignment from a 1D initializtion list.
    //
    template<class U>
    constexpr matrix& operator =(initializer_list<U> rhs);
    ...
    //----------------------------------------------------------
    //- Custom operation traits injection.
    //
    template<class COT2>
...
template<class T, ptrdiff_t R, ptrdiff_t C, class A = allocator<T>, class COT = void>
...
template<class T, class COT = void> 
    using dynamic_matrix =
        matrix<matrix_storage_engine<T, 
...
template<class T, ptrdiff_t R, class A = allocator<T>, class COT = void>
    using general_column_vector =
        matrix<matrix_storage_engine<T, extents<R, 1>, A, matrix_layout::column_major>, COT>;

template<class T, ptrdiff_t C, class A = allocator<T>, class COT = void>
...
template<class T, class COT = void> 
    using dynamic_column_vector =
        matrix<matrix_storage_engine<T, extents<dynamic_extent, 1>, allocator<T>, matrix_layout::column_major>, COT>;

template<class T, class COT = void> 
    using dynamic_row_vector =
        matrix<matrix_storage_engine<T, extents<1, dynamic_extent>, allocator<T>, matrix_layout::row_major>, COT>;
~~~~
...
class basic_vector
{
  public:
    ...
    using subvector_type       = basic_vector<matrix_view_engine<engine_type, /* See note */>, COT>;
  ...
  public:
    ...
    //
    template<class ET2, class COT2>
    constexpr explicit basic_vector(matrix<ET2, COT2> const& rhs);
    ...
    //
    template<class ET2, class COT2>
    constexpr basic_vector& operator =(matrix<ET2, COT2> const& rhs);
